...
db_file_path = os.path.abspath(os.getcwd())+"\database\jems_db.db"
...
def jems_beer_calculate_page():
    ...
    if request.method == 'POST':
        ...
        page = Page(shift.date, managers, shift.selected_shift, shift.total_cash, shift.total_credit,
                    shift.total_hours, shift.cash_per_hour, shift.credit_per_hour, shift.total_tip,
                    data_lists["waiters_name"],
            ...
            if money_shift is not None:
                if money_shift.date == datetime.fromisoformat(data["date"]).date():
                    if money_shift.selected_shift == data["shift"]:
                        ...
                        page = Page(data["date"], money_shift.manager, data["shift"], money_shift.total_cash,
                                    money_shift.total_credit,
                                    money_shift.total_hours, money_shift.cash_per_hour, money_shift.credit_per_hour,
                                    money_shift.total_tip, data_lists["waiters_name"],
                                    data_lists["start_time_waiter"], data_lists["finish_time_waiter"],
                                    data_lists["total_waiter_time"],
                                    data_lists["total_cash_waiter"], data_lists["total_credit_waiter"],
                                    data_lists["total_tips_waiter"])
                        page_json = calculate_schema.dump(page)
                        union_shift_data = dict(page_json,
                                                **({"shift_exist": True, "msg": "Shift found", "shift": "morning"}))
